...
void extract_dib_header_data(DibHeader *dib_header, unsigned char *dib_header_data)
{
    ...
    for(i = 0; i < 4; i++) {
        pixel_array_size[i] = dib_header_data[byte_array_size_start_position + i];
    }
    dib_header->pixel_array_size = lsb_to_int(pixel_array_size);
    printf("byte array size=%d\n", dib_header->pixel_array_size);

    unsigned char bmp_width[4];
    int bmp_width_start_position = 0;
    for(i = 0; i < 4; i++) {
        bmp_width[i] = dib_header_data[bmp_width_start_position + i];
    }
    dib_header->img_width = lsb_to_int(bmp_width);
    printf("width=%d\n", dib_header->img_width);

    unsigned char bmp_height[4];
    int bmp_height_start_position = 4;
    for(i = 0; i < 4; i++) {
        bmp_height[i] = dib_header_data[bmp_height_start_position + i];
    }
    dib_header->img_height = lsb_to_int(bmp_height);
